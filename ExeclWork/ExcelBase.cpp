...
void ExcelBase::setSheetName(QString SheetName)
{
#if defined(Q_OS_WIN)
	Q_D(ExcelBase);
	if (d->excel != NULL && !d->excel->isNull())
...
void ExcelBase::addSheet(QString SheetName)
{
#if defined(Q_OS_WIN)
	Q_D(ExcelBase);
	if (d->excel != NULL && !d->excel->isNull())
	{
		int nsheetCount = sheetCount();
		QAxObject * lastSheet = d->sheets->querySubObject("Item(int)", nsheetCount);
		QAxObject* workSheet = d->sheets->querySubObject("Add(QVariant)", lastSheet->asVariant());
		d->sheet->dynamicCall("Move(QVariant)", workSheet->asVariant());
