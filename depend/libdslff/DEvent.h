...
public:
	...
	int SetEvent() { Lock(); int ret = SetEventInLock(); Unlock(); return ret; }
	int Reset();

	int Wait() { return TimedWait( -1 ); }
	int TimedWait( int timeout_ms ) { Lock(); int ret = TimedWaitInLock(timeout_ms); Unlock(); return ret; }

	void Lock() { m_lock.Lock(); return; }
	void Unlock() { m_lock.Unlock(); return; }
