...
public:
	...
	bool IsStopping() { return m_state == DSL_THREAD_STATE_STOPPING; }
	bool IsIdle() { return m_state == DSL_THREAD_STATE_IDLE; }
...
// 当使用DSL_RUNNER_TYPE_RUN类型的runner时，需要在线程函数中定期检查线程是否需要退出，或者使用其他途径通知

enum dsl_runner_type_e {
	DSL_RUNNER_TYPE_UNKNOWN = 0,
	DSL_RUNNER_TYPE_RUN,
	DSL_RUNNER_TYPE_HEARTBEAT,
};
...
public:
	...
	DRunner( func_type_t func = NULL, T * obj = NULL, dsl_runner_type_e type = DSL_RUNNER_TYPE_RUN )
		: m_func( func ), m_obj( obj ), m_type( type ) { }

	inline int SetRunner( func_type_t func, T * obj, dsl_runner_type_e type = DSL_RUNNER_TYPE_RUN )
	{
		m_func = func;
		m_obj = obj;
		m_type = type;
...
protected:
	virtual inline int run()
	{
		...
		if( m_type == DSL_RUNNER_TYPE_RUN ) {
			return (m_obj->*m_func)( this );
		}
		else if( m_type == DSL_RUNNER_TYPE_HEARTBEAT ) {
			return DThread::run();
		}
		return -1;
	}
...
template<>
class DRunner<void> : public DThread
{
public:
	DRunner( dsl_runner_func_t func = NULL, void * arg = NULL, dsl_runner_type_e type = DSL_RUNNER_TYPE_RUN )
		: m_func( func ), m_arg( arg ), m_type( type ) { }

	inline int SetRunner( dsl_runner_func_t func, void * arg = NULL, dsl_runner_type_e type = DSL_RUNNER_TYPE_RUN )
	{
		m_func = func;
		m_arg = arg;
		m_type = type;
